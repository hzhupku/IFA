...
def get_scheduler(cfg_trainer, len_data, optimizer, start_epoch=0, use_iteration=False):
    epochs = cfg_trainer['epochs'] if not use_iteration else 1  # if use_iteration = True, only one epoch be use
    lr_mode = cfg_trainer['lr_scheduler']['mode']
    lr_args = cfg_trainer['lr_scheduler']['kwargs']
    lr_scheduler = LRScheduler(lr_mode, lr_args, len_data, optimizer, epochs, start_epoch)
    return lr_scheduler
...
class LRScheduler(object):
    def __init__(self, mode, lr_args, data_size, optimizer, num_epochs, start_epochs):
        ...
        if mode == 'milestones':
            ...
            self.milestones = lr_args['milestones'] if lr_args.get('milestones', False) else default_mist
    ...
    def _step(self):
        if self.mode == 'step':
            ...
            for i, lr in enumerate(self.base_lr):
                adj_lr = lr*((1 - float(self.cur_iter) / self.max_iter)**(self.power))
                self.cur_lr[i] = adj_lr
        elif self.mode == 'cosine':
            for i, lr in enumerate(self.base_lr):
                adj_lr = self.targetlr + (lr - self.targetlr) * (1 + cos(pi * self.cur_iter / self.max_iter)) / 2
                self.cur_lr[i] = adj_lr
        else:
